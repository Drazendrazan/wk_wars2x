...
AddEventHandler('QBCore:Client:OnPlayerLoaded',function()
    ...
    TriggerServerEvent('dispatch:forzarlog')
end)

RegisterNetEvent('dispatch:GetPlayerGroup')
AddEventHandler('dispatch:GetPlayerGroup',function(PlayerGroup)
...
AddEventHandler("onResourceStart", function(resource)
 ...
 RegisterNetEvent("dispatch:alertToClient")
 AddEventHandler("dispatch:alertToClient", function(text, coords, id)
     local id_ = id
...
AddEventHandler("dispatch:alertToClient", function(text, coords, id)
    ...
    if Player ~= nil then
        if Player.job.name ~= nil then
            if (Player.job.name == 'police' and (activated and Player.job.onduty) and activated and toggleAlertsAdmin)  then
                callnum = callnum + 1
                totalcalls = totalcalls + 1
...
RegisterNetEvent("dispatch:vehToClient")
AddEventHandler("dispatch:vehToClient", function(coords, model, cl1, cl2, id, plate)
    id = id
    local Player = QBCore.Functions.GetPlayerData()
    local ped = PlayerPedId()
    local pedo = PlayerPedId(id)
    local mycoords = GetEntityCoords(ped)
    local veh = GetVehiclePedIsUsing(PlayerPedId(id))
    local color = Config.hks_Colors[cl1]
    local predist = #(mycoords - coords)
    local dist = math.ceil(predist)
    ...
    if Player ~= nil then
        if PlayerJob ~= nil then
            if PlayerJob.name == 'police' and activated then
                callnum = callnum + 1
                totalcalls = totalcalls + 1
                local distanceToAlert = dist
                local finalDistanceTo = distanceToAlert
                local id_ = id
                local streetnames = GetStreetNames(coords)
                if Config.enableVehiclePics then
                    SendNUIMessage({
                        content = "["..id_.."] Una persona ha robado un " ..model.. " de color <div style='width:30px;height:15px;display:inline-block;background-color:rgb(" ..color.r.. "," ..color.g.. "," ..color.b.. ")'></div>, Matriclua "..plate..", Te encuentras a " ..dist .. " m al momento del aviso";
                        callnum = callnum;
                        totalcalls = totalcalls;
                        newalert = true;
                        id = id;
                    })
                    table.insert(calls, {text = "Una persona ha robado un " ..model.. " de color  <div style='width:30px;height:15px;display:inline-block;background-color:rgb(" ..color.r.. "," ..color.g.. "," ..color.b.. ")'></div>, Matriclua "..plate.." , Al momento de la llamanda estabas a " ..finalDistanceTo .. " m de distancia", coords = coords, model = model})
                else
                    if dentrozonasur then
                        SendNUIMessage({
                        ...
                        SendNUIMessage({
                            content = "<FONT SIZE=2> <a style= 'color:#E3AE13'>Aviso LSPD ["..id_.."] </font> </a> <br /> <FONT SIZE=2> Acaban de robar un vehículo modelo " ..model.. " de color <div style='width:30px;height:15px;display:inline-block;background-color:rgb(" ..r.. "," ..g.. "," ..b.. ")'></div> Con matrícula "..plate..". </font> <br /> <FONT SIZE=1>" ..streetnames.street1..", " ..streetnames.street2.. '</font> <br /> ';
                            callnum = callnum;
                            totalcalls = totalcalls;
                            newalert = true;
                            id = id;
...
RegisterNetEvent("dispatch:vehToClientspeed")
AddEventHandler("dispatch:vehToClientspeed", function(coords, model, cl1, cl2, id, plate)
    id = id
    local Player = QBCore.Functions.GetPlayerData()
    local ped = PlayerPedId()
    local pedo = PlayerPedId(id)
    ...
    if Player ~= nil then
        if PlayerJob ~= nil then
            if PlayerJob.name == 'police' and activated then
                ...
                if Config.enableVehiclePics then
                    SendNUIMessage({
                        content = "["..id_.."] Un " ..model.. " de color <div style='width:30px;height:15px;display:inline-block;background-color:rgb(" ..color.r.. "," ..color.g.. "," ..color.b.. ")'></div>, Matriclua "..plate.." Se a visto a grandes a granvelocidad. Te encuentras a " ..dist .. " m al momento del aviso";
                        callnum = callnum;
                        totalcalls = totalcalls;
                        newalert = true;
                        id = id;
                    })
                    table.insert(calls, {text = "Un " ..model.. " de color  <div style='width:30px;height:15px;display:inline-block;background-color:rgb(" ..color.r.. "," ..color.g.. "," ..color.b.. ")'></div>, Matriclua "..plate.." Se a visto a grandes velocidades. Al momento de la llamanda estabas a " ..finalDistanceTo .. " m de distancia", coords = coords, model = model})
                else
                    if dentrozonasur then
                        SendNUIMessage({
                            content = "<FONT SIZE=2> <a style= 'color:#2A73FC'>Aviso LSPD ["..id_.."] </font> </a> <br /> <FONT SIZE=2> Acaban de robar un vehículo modelo " ..model.. " de color <div style='width:30px;height:15px;display:inline-block;background-color:rgb(" ..r.. "," ..g.. "," ..b.. ")'></div> Con matrícula "..plate..". </font> <br /> <FONT SIZE=1>" ..streetnames.street1..", " ..streetnames.street2.. '</font> <br /> ';
...
RegisterNetEvent("dispatch:auxToClient")
AddEventHandler("dispatch:auxToClient", function(text, coords, id)
...
RegisterNetEvent("dispatch:taxiToClient")
AddEventHandler("dispatch:taxiToClient", function(text, coords, id, phone)
    if PlayerJob.name == 'taxi' and activated then
        callnum = callnum + 1
        totalcalls = totalcalls + 1
        local PlayerPhone2 = phone
        local streetnames = GetStreetNames(coords)
        SendNUIMessage({
            content = text.. "<br />Tlf: " ..PlayerPhone2.. "<br />" ..streetnames.street1..", " ..streetnames.street2;
            callnum = callnum;
            totalcalls = totalcalls;
            newtaxialert = true;
            newalert = true;
...
RegisterNetEvent("dispatch:mecaToClient")
AddEventHandler("dispatch:mecaToClient", function(text, coords, id, phone)
    if PlayerJob.name == 'lscustom' or PlayerJob.name == 'mechanic' or PlayerJob.name == 'flywheels' or PlayerJob.name == 'paletomonkey' or PlayerJob.name == 'northwayautos' and activated then
        callnum = callnum + 1
        totalcalls = totalcalls + 1
        local PlayerPhone2 = phone
        local streetnames = GetStreetNames(coords)
...
RegisterNetEvent("dispatch:robberyToClient")
AddEventHandler("dispatch:robberyToClient", function(type, coords, id)
    local id_ = id
    if PlayerJob.name == 'police' and activated then
        callnum = callnum + 1
        totalcalls = totalcalls + 1
...
RegisterNetEvent('dispatch:createblip')
AddEventHandler('dispatch:createblip', function(Coords, id, type)
        ...
        if type == 'entorno' or type == 'forzar' or type == 'robobancocentral' or type == 'robojoyeria' or type == 'roboyate' or type == 'robolife' or type == 'robopelu' or type == 'roboammu' or type == 'robotienda' or type == 'fleeca' or type == 'ventadroga' or type == 'roboimportación' then
            if PlayerJob ~= nil then
                if PlayerJob.name == 'police' and activated then
                    ...
                    if type == 'entorno' then -- llamada de un ciudadano a la polica
                        SetBlipSprite(call, 66)
                        SetBlipColour (call, 44)
                    elseif type == 'forzar' then -- robo de vehiculo
                        SetBlipSprite(call, 326)
                        SetBlipColour (call, 334)
                    elseif type == 'robobancocentral' then -- robo banco central
                        SetBlipSprite(call, 605)
                        SetBlipColour (call, 1)
                    elseif type == 'robojoyeria' then
                        SetBlipSprite(call, 617)
                        SetBlipColour (call, 1)
            ...
            elseif type == 'meca' then
                if PlayerJob.name == 'lscustom' or PlayerJob.name == 'mechanic' or PlayerJob.name == 'flywheels' or PlayerJob.name == 'paletomonkey' or PlayerJob.name == 'nortwayautos' or PlayerJob.name == 'hayesauto' or PlayerJob.name == 'mayerautos' and activated then
                    local alpha = 250
                    local call = AddBlipForCoord(targetCoords.x, targetCoords.y, targetCoords.z)
                    if type == 'meca' then
                        SetBlipSprite(call, 162)
...
RegisterCommand("rightbl", function()
    print "derecha"
...
RegisterCommand("leftbl", function()
    if calls[callnum - 1] ~= nil then
        local num = callnum - 1
        if calls[callnum - 1]['model'] ~= nil then
            SendNUIMessage({
...
RegisterCommand("moverbl", function(source, args)
    if showed then 
...
RegisterCommand("acceptentornobl", function(source, args)
    if showed then
...
RegisterKeyMapping("moverbl", ("Configuración"), 'keyboard', 'y')

RegisterKeyMapping("rightbl", ("Mover entorno a la derecha"), 'keyboard', 'right')

RegisterKeyMapping("leftbl", ("Mover entorno a la izquierda"), 'keyboard', 'left')

RegisterKeyMapping("showalertsbl", ("Abrir el dispatch"), 'keyboard', 'u')

RegisterKeyMapping("acceptentornobl", ("Marcar el entorno"), 'keyboard', 'o')
...
RegisterNetEvent('dispatch:client:togglealertsadmin')
AddEventHandler('dispatch:client:togglealertsadmin', function(boolean)
